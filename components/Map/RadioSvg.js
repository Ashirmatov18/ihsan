...
class RadioSVGMap extends React.Component {
  ...
  componentDidMount() {
    ...
    this.parents = Array.from(this.locations).filter(
      (item) => !item.id.split("_").includes("Vector")
    );

    this.pointers = Array.from(this.locations).filter((item) =>
      item.id.split("_").includes("Vector")
    );
  ...
  selectLocation(location) {
    ...
    if (this.isPointerOrLake(location)) {
      const parent = Array.from(this.parents).find((item) =>
        location.id.includes(item.id)
      );
      parent.focus();
    } else {
      pointer = Array.from(this.pointers).find((item) =>
        item.id.split("_").includes(location.id)
      );
      location.focus();
    }
